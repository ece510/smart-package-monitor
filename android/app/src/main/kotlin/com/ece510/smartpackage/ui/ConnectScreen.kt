...
fun ConnectScreen(
    ...
    Column(
        ...
        Text(
            "Conecta con el paquete para ver el historial del trayecto.",
        ...
        when {
            !hasPermission -> StatusMessage(
                "Se necesita el permiso de Bluetooth para continuar.",
            ...
            !isBluetoothEnabled() -> StatusMessage(
                "Bluetooth está apagado. Actívalo desde los ajustes del teléfono.",
            )

            state is AppUiState.Connecting -> LoadingMessage("Conectando con el paquete…")

            state is AppUiState.Downloading -> LoadingMessage("Descargando el historial del trayecto…")
                ...
                if (devices.isEmpty()) {
                    StatusMessage(
                        "No hay dispositivos emparejados. Empareja el paquete desde " +
                            "los ajustes de Bluetooth del teléfono y vuelve a abrir la app.",
                    )
                } else {
        ...
        items(devices) { device ->
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    Text(
                        runCatching { device.name }.getOrNull() ?: "Dispositivo sin nombre",
