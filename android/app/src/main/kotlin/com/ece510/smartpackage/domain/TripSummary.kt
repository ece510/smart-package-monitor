...
fun reasonLabel(code: String): String = when (code) {
    ...
    "CV" -> "Tampering detected by camera"
    else -> code
}

/** Color associated with an alert reason code, used for the "Alerts by type" chips. */
fun reasonColor(code: String): Color = when (code) {
    "ACCEL" -> Color(0xFFC62828) // red — matches the chart's alert markers
    "TEMP" -> Color(0xFFEF6C00) // orange
    "HUM" -> Color(0xFF1565C0) // blue
    "CV" -> Color(0xFF6A1B9A) // purple
    else -> Color(0xFF616161) // gray fallback
}
